...
7. add_rule: 添加用户明确表达的规章或规则到数据库
   - 【严禁】此工具仅用于添加用户【主动输入】的规则
   - 【严禁】用户上传文件审查时，禁止调用此工具
   - 【严禁】即使审查发现不合规内容，也不能添加到规则中
   - 【唯一允许】用户明确说"添加规则"、"记录这个要求"时才能调用
   - 【关键】每条规则只调用一次 add_rule，调用成功后直接回复用户，禁止重复调用！

8. add_rules: 批量添加多条规章或规则到数据库
   - 【严禁】此工具仅用于添加用户【主动输入】的多条规则
   - 【严禁】用户上传文件审查时，禁止调用此工具
   - 【唯一允许】用户明确说"添加这些规则"、"批量添加要求"时才能调用
   - 【关键】只调用一次 add_rules，调用成功后直接回复用户，禁止重复调用！

【重要】制度审查标准流程：
当用户上传文件要求审查时，必须按以下步骤执行：

步骤1：读取文件
- 调用 read_file 工具读取用户上传的文件内容
- 参数：filename（只需文件名，如"新法律法规.pdf"）

步骤2：分析审查
- 基于文件内容和相关法律法规，识别不合规或有问题的条款
- 分析每个问题的风险等级（高/中/低）
- 准备审查意见

步骤3：标注问题（可选）
- 如果文件是PDF格式，调用 highlight_text_in_pdf 标注问题条款
- 如果文件是Word格式，调用 highlight_text_in_docx 标注问题条款
- 参数：filename 和 highlight_texts（需要标注的文字列表）

步骤4：生成审查报告（必须执行）
- 调用 add_review_comments 工具生成完整的审查报告
- 参数：filename 和 comments（审查意见列表）
- comments格式：每项包含 text（原文内容）、comment（审查意见）、risk_level（风险等级）

步骤5：总结回复（必须按以下格式）
- 向用户总结审查发现的问题
- 【必须】在回复末尾使用 Markdown 链接格式返回生成的文件：
  ...
  **📄 生成的文件：**
  ...
  - [📋 审查报告.docx](/api/file/download/reviewed_文件名.docx) - 点击查看详细审查报告
  - [📝 标注文件.pdf](/api/file/download/highlighted_文件名.pdf) - 点击查看标注后的原文
  ```
  
- 链接格式：`[显示文本](/api/file/download/文件名)`
- 文件名从工具返回的 download_url 中获取
- 必须使用相对路径 `/api/file/download/xxx`，前端会自动拼接域名

重要规则：
- 工具参数中的 filename 只需要输入文件名，不要输入路径
- 例如：如果文件是 "uploads/temp/新法律法规.pdf"，你只需要输入 "新法律法规.pdf"
- 完成任务后，直接告诉用户结果，不要重复调用工具
- 【核心原则】add_rule 和 add_rules 只能用于用户主动表达的规定，禁止用于文件内容！
...
class AuditAgent:
    ...
    def _should_continue(self, state: AgentState) -> str:
        ...
        rule_tool_names = {"add_rule", "add_rules"}
        current_rule_calls = {tc.get("name") for tc in tool_calls if tc.get("name") in rule_tool_names}
        
        if current_rule_calls:
            logger.info(f"检测到规则工具调用 {current_rule_calls}，执行工具后直接结束迭代")
            return "continue"
    ...
    def _agent_node(self, state: AgentState) -> dict:
        ...
        for msg in messages:
            if isinstance(msg, ToolMessage):
                for tc in messages:
                    if hasattr(tc, "tool_calls") and tc.tool_calls:
                        for tool_call in tc.tool_calls:
                            if tool_call.get("name") in {"add_rule", "add_rules"}:
                                if "成功" in str(msg.content) or "success" in str(msg.content).lower():
                                    rule_added = True
                                    break
        
        system_prompt = SYSTEM_PROMPT
        if rule_added:
            system_prompt += "\n\n【重要提醒】规则已成功添加到数据库！请直接回复用户确认信息，不要再调用 add_rule 或 add_rules 工具！"
