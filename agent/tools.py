...
class AddRuleTool(BaseTool):
    ...
    name: str = "add_rule"
    description: str = """【严禁滥用】此工具仅用于添加用户【主动输入】的规则，禁止用于其他任何场景！
...
❌ 审查发现不合规内容 → 禁止调用此工具
❌ 文件中有需要修改的地方 → 禁止调用此工具
❌ 用户没有明确说"添加规则" → 禁止调用此工具

【唯一允许的场景】
✅ 用户在对话中明确说："添加一个规则：xxx" 或 "记录这个要求：xxx"
✅ 用户主动表达要保存某个规定或要求

【正确示例】
用户输入："帮我添加一个规则：科研成果由个人自行处置"
→ 可以调用：add_rule(title="科研成果处置规则", content="科研成果由个人自行处置...", rule_type="global")

用户输入："记录一下这个要求：所有合同必须法务审核"
→ 可以调用：add_rule(title="合同审核要求", content="所有合同必须经过法务部门审核", rule_type="global")

【错误示例 - 绝对禁止】
用户上传文件说："帮我审查这份文件"
→ 禁止调用 add_rule！即使发现不合规内容也不能添加到规则！

用户上传文件说："这个文件有问题，帮我看看"
→ 禁止调用 add_rule！
...
class AddRulesTool(BaseTool):
    ...
    name: str = "add_rules"
    description: str = """【严禁滥用】此工具仅用于添加用户【主动输入】的多条规则，禁止用于其他任何场景！
...
【唯一允许的场景】
✅ 用户明确说："添加这些规则：xxx、xxx、xxx" 或 "批量添加以下要求"
✅ 用户主动表达要保存多个规定或要求

【正确示例】
用户输入："帮我添加这些规则：1.科研成果由个人处置 2.所有合同需法务审核"
→ 可以调用此工具
